...
type ToolCall struct{Params []string; Tool Tool; Result string}
...
func agentHandler(ctx LoopContext) (LoopResponse, error) {
	...
	if len(ctx.toolCall) != 0 {
		...
		return LoopResponse{
			answer: fmt.Sprintf("exec tool: %v\n", ctx.toolCall),
		}, nil
	} else if ctx.query != "" {
		...
		return LoopResponse{}, fmt.Errorf("empty LoopMessage: %v", ctx)
	}
...
func agentLoop(input io.Reader, output io.Writer) int {
	for {
...
func main() {
	os.Exit(agentLoop(os.Stdin, os.Stdout))
}
